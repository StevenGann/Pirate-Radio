...
- **RESUME POINT: build P6-6** — Phase-6 housekeeping + full-seven deep-dive (must ratify the R8′ ring-buffer deviation + skip-at-next-boundary semantics), then remediate any CRITICAL/HIGH. Then the final full-team code-quality + documentation review (overnight mandate's closer).
