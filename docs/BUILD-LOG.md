...
- **P6-3 ✅** Control paths + seams: **skip-at-next-boundary** Event threaded Player←run_once←play_day←Station (drops next item at loop top, one-shot; no mid-segment cut); per-station **regen `asyncio.Lock`** shared by MidnightTask + `Coordinator.regenerate_station` (offload via injected to_thread, R23; on-disk, effect next roll); `Coordinator.skip`/`regenerate_station`; `ControlService.skip`/`regenerate` (validate→StationNotFound before invoking) — `0059`. Gate: 826 tests, 97.60%.
- **P6-4 ✅** `control/api.py` `create_app` (§15 routes; bearer auth constant-time + env-name fail-fast; `{success,data,error}` envelope via exception handlers 404/401/422; skip/regenerate 202; R23 reads via injected offload; `/logs` from the ring) — `0060`. TestClient + `disable_socket` (+ --allow-unix-socket) proves no TCP bind (R21). Gate: 838 tests, 97.62%.
- **P6-5 ✅** `control/server.py` (`make_server` builds `uvicorn.Server` no-bind, unit-testable host/port; `serve` = the real bind, pragma'd) + `config.ControlConfig` (enabled/host `127.0.0.1`/port 8080/token_env/log_ring_size; `DaemonConfig.control: ControlConfig|None=None` ⇒ absent means no API) + `Coordinator.build_control_service` (wires ControlService over live registry/configs/clock/skip/regenerate) + `__main__` crash-isolated API task (`MainDeps.build_api`; `_run_daemon` gathers `coordinator.run()` ‖ `_isolated(api_coro)`; `_isolated` re-raises CancelledError but swallows+logs other crashes so the broadcast outlives an API failure, H-A5) — `0061`. Integration-glue increment (folds prior-panel lessons; P6-6 deep-dive is the backstop). Runbook: `docs/ops/control-api.md` (loopback+SSH-tunnel default, token-by-name in 0600 EnvironmentFile + rotation, leak-safe curl, endpoint table w/ skip-at-boundary + regen-at-next-roll, `/logs` ring lossy-across-restarts caveat). Gate: 843 tests, 97.33%, ruff/mypy clean (61 files).
- **P6-6 ✅** Full-seven Phase-6 deep-dive + remediation — `0062`. Round 1: CONFIRM ×4 (Senior Dev, Old Man, RPi, Fact Checker — both ratification asks RATIFIED), DISPUTE ×3 (DA, QA, Field-Op) finding 2 CRITICAL + 7 HIGH in the seams unit tests don't cross. Remediated all: clean-exit visibility (`_isolated` logs on ANY exit + runbook "Detecting a dead control plane"); ring thread-safety pinned by a mutation-sensitive lock-acquisition test; stale-skip cleared at the slice boundary; "skip never cuts airing" test; un-enveloped 500s closed (naive-`since` coercion + non-ASCII-auth fail-closed + catch-all envelope handler); ring INFO floor (RPi hot-path); regen-lock-vs-real-MidnightTask end-to-end test; auth malformed-scheme tests; `ControlConfig` tests + `log_ring_size`/`limit` caps; runbook ufw/troubleshooting/skip-nuance + `state_dir`=`StateDirectory` note. Re-poll: DA + Field-Op + QA all flipped to **CONFIRM** (QA ran its own mutation checks incl. a per-site partial). **All 7 CONFIRM.** Both ASKS unanimous: R8′ ring (RAM-only/H26/R23-safe/journald source-of-truth) + skip-at-next-boundary RATIFIED. Gate: 869 tests (+26), 97.34%, ruff/mypy clean (61 files).
